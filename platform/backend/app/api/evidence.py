...
def list_evidence(session: Session = Depends(get_session), current_user=Depends(get_current_user)):
    return session.exec(select(RawEvidence)).all()


@router.post("/", response_model=RawEvidenceRead)
def create_evidence(item: RawEvidenceCreate, session: Session = Depends(get_session), current_user=Depends(get_current_user)):
...
def get_evidence(evidence_id: int, session: Session = Depends(get_session), current_user=Depends(get_current_user)):
